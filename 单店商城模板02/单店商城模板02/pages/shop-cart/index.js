    ...
    toPayOrder:function(){
      ...
      for (let i =0; i < shopList.length; i++) {
        ...
        if (!carShopBean.propertyChildIds || carShopBean.propertyChildIds == "") {
          wx.request({
            url: 'https://xcx.shty888.com/product.info',
            data: {
            ...
            success: function(res) {
              ...
              //   wx.showModal({
              //     title: '提示',
              //     content: res.data.data.product.name + ' 商品已失效，请重新购买',
              //     showCancel:false
              //   })
              //   isFail = true;
              //   wx.hideLoading();
              //   return;
              // }
              if (res.data.data.productAttribute.stock < carShopBean.number) {
                wx.showModal({
                  title: '提示',
                  content: res.data.data.product.name + ' 库存不足，请重新购买',
                  showCancel:false
                })
                isFail = true;
                wx.hideLoading();
              ...
              if (res.data.data.product.showPrice != carShopBean.price) {
                wx.showModal({
                  title: '提示',
                  content: res.data.data.product.name + ' 价格有调整，请重新购买',
                  showCancel:false
                })
                isFail = true;
                wx.hideLoading();
