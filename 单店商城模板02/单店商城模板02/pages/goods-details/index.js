...
Page({
  ...
  onLoad: function (e) {
    ...
    this.productId = e.id;
    wx.request({
      url: 'https://xcx.shty888.com/product.info?productId=' + e.id,
 
      success: function(res) {
        var selectSizeTemp = "";
        if (res.data.data.properties) {
          for(var i=0;i<res.data.data.product.length;i++){
            selectSizeTemp = selectSizeTemp + " " + res.data.data.Product[i].name;
          }
          that.setData({
            hasMoreSelect:true,
            selectSize:that.data.selectSize + selectSizeTemp,
            selectSizePrice: res.data.data.product.showPrice,
          });
        }
        ...
        that.setData({
          ...
          selectSizePrice: res.data.data.product.showPrice,
          buyNumMax: res.data.data.productAttribute.stock,
          buyNumber: (res.data.data.productAttribute.stock>0) ? 1: 0
        });
  ...
  tobuy: function () {
    ...
    this.bindGuiGeTap(); 
    if (this.data.goodsDetail.productAttribute.stock< 1){
          wx.showModal({
            title: '提示',
            content: '暂时缺货哦~',
            showCancel:false
    ...
    */
    ...
    if (canSubmit) {
      wx.request({
        ...
        data: {
          goodsId: that.data.goodsDetail.product.productId,
  ...
  */
  ...
  bulidShopCarInfo: function () {
    // 加入购物车
    var shopCarMap = {};
    shopCarMap.goodsId = this.data.goodsDetail.product.productId;
    shopCarMap.pic = this.data.goodsDetail.product.picImg;
    shopCarMap.name = this.data.goodsDetail.product.name;
